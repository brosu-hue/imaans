#!/usr/bin/env node
// magic-shot: tools/shot.mjs + per-camera JS (for burst / sparkle screenshots). Magic module dev tool.
//   --cams "burst:0,1.62,2.5,0,1.4,-2@__ctx.fx.burst(new __ctx.THREE.Vector3(0,1.4,-2),{age:0.35})"
//   --pre "js"   run before every camera (after it is set);  --frames N  extra frames to wait per camera
...
const cams = String(args.cams || 'start:0,1.62,8.4,0,1.45,-4').split(';').filter(Boolean).map(s => {
  const at = s.indexOf('@'); const js = at >= 0 ? s.slice(at + 1) : null; if (at >= 0) s = s.slice(0, at);
  const [name, v] = s.split(':'); return { name, v: v.split(',').map(Number), js };
});
...
if (args.ui) {
  ...
  const f = path.join(outDir, 'ui.png'); await page.screenshot({ path: f, timeout: Number(args.shotTimeout || 180000) }); shots.push(f);
}
if (!args.ui || args.cams) {
  for (const c of cams) {
    await page.evaluate(v => window.__setCam && window.__setCam(...v), c.v);
    if (args.pre) await page.evaluate(String(args.pre)).catch(e => logs.push('[pre] ' + e.message));
    if (c.js) await page.evaluate(c.js).catch(e => logs.push('[cam js] ' + e.message));
    await page.evaluate(() => new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r))));
    for (let i = 0; i < Number(args.frames || 0); i++) await page.evaluate(() => new Promise(r => requestAnimationFrame(r)));
    await page.waitForTimeout(Number(args.wait || 400));
    const f = path.join(outDir, c.name + '.png');
    await page.screenshot({ path: f, timeout: Number(args.shotTimeout || 180000) });
    shots.push(f);
  }
}
let evalResult;
