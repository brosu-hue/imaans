...
//   --no-prefetch     src only: don't inject the boot model-prefetch list (tools/perf-models.mjs) — A/B timing
...
// dev pages get the same boot model-prefetch list the production bundle bakes in (see src/main.js PREFETCH)
if (args.src !== 'dist' && !args['no-prefetch']) {
  let pre = null; try { pre = scanModels(ROOT).prefetch; } catch (e) { console.error('[shot] prefetch scan failed:', e.message); }
