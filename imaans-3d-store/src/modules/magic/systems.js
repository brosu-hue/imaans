...
/** (1) ambient gold dust — a share of it inside the spot beams (and brighter there). */
export function buildDust(ctx, n, beams, r) {
  const { ROOM } = ctx.layout;
  const buf = new Buf(n);
  const wsum = beams.reduce((s, b) => s + b.w, 0);
  const nBeam = beams.length ? Math.round(n * 0.4) : 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i < nBeam) {
      ...
      p = [r.range(ROOM.minX + 0.4, ROOM.maxX - 0.4), 0.25 + 4.0 * Math.pow(r(), 0.9), r.range(ROOM.minZ + 0.4, ROOM.maxZ - 0.5)];
    }
    const col = pickColor(r, DUST_COLORS);
    const k = col === GOLD.deep ? 0.55 : 0.8;
    buf.push(p, [r(), r.range(0.024, 0.05), r.range(0.1, 0.42), r()], [boost, r() < 0.16 ? 1 : 0, 0, KIND.dust], lin(col, k * 2.2));
  }
  return buf;
}

/** (2) gold helix spiralling up around the Spring Edit plinth + glitter drifting down from the cove. */
export function buildPlinth(ctx, nHelix, nGlitter, r) {
  ...
  const { cx, cz } = ctx.layout.ZONES.plinth;
  for (let i = 0; i < nGlitter; i++) {
    const rad = Math.sqrt(r.range(1.05 * 1.05, 2.2 * 2.2)), a = r() * Math.PI * 2;
...
/** (3) soft bokeh orbs wandering at 0.5–3.5 m. */
export function buildBokeh(ctx, n, r) {
  const buf = new Buf(n);
  const table = [[GOLD.light, 0.5], [GOLD.cream, 0.3], [GOLD.mid, 0.12], [GOLD.rose, 0.05], [GOLD.aqua, 0.03]];
  for (let i = 0; i < n; i++) {
    // keep them where people look: the aisle, the plinth, the salon, the tables
    const x = r.range(-6.4, 6.4) * (r() < 0.5 ? 0.6 : 1), z = r.range(-9.6, 8.0), y = r.range(0.95, 3.1);
    buf.push([x, y, z], [r(), r.range(0.045, 0.095), r.range(0.55, 1.5), r()], [0, 0, 0, KIND.bokeh], lin(pickColor(r, table), r.range(0.14, 0.3)));
  }
  return buf;
}

/**
 * (5) shimmer sweeping across the IMAANS wordmark (+ the central shoe niches): glints that light up as
 * the band passes, and a few soft sheen discs travelling with it.
 */
export function buildShimmer(ctx, n, r) {
  const buf = new Buf(n + 9);
  const Z = -10.9;   // just in front of the halo-lit letters (z ≈ -10.955)
  for (let i = 0; i < n; i++) {
    const u = r();
    let p, idle = 0;
    if (u < 0.6) p = [r.range(-0.99, 0.99), r.range(3.75, 4.06), Z];            // the IMAANS capitals
    else if (u < 0.68) p = [r.range(-0.13, 0.13), r.range(4.12, 4.29), Z];       // the crown
    else if (u < 0.76) p = [r.range(-0.72, 0.72), r.range(3.63, 3.69), Z];       // SHOES & CLOTHING
    else { p = [r.range(-3.4, 3.4), r.range(0.55, 3.35), -10.42]; }              // shoe-niche fronts
    if (u < 0.68 && r() < 0.2) idle = 1;                                          // a few glint now and then
    const col = pickColor(r, [[GOLD.light, 0.5], [GOLD.white, 0.35], [GOLD.mid, 0.15]]);
    buf.push(p, [r(), r.range(0.03, u < 0.76 ? 0.075 : 0.05), 0, r()], [idle, 0, 0, KIND.glint], lin(col, u < 0.76 ? 2.8 : 1.4));
  }
  for (let i = 0; i < 9; i++) {
    buf.push([-1.0 + i * 0.25, 3.9 + (i % 2) * 0.06, Z - 0.02], [r(), 0.46, 0, i / 9 * 0.5], [0, 0, 0, KIND.sheen], lin(GOLD.light, 1.0));
  }
  return buf;
}
...
export function buildCritters(ctx, r) {
  ...
  const st = Z.shoeStage, wl = Z.windowLeft, wr = Z.windowRight;
  const wlx = (wl.x[0] + wl.x[1]) / 2, wrx = (wr.x[0] + wr.x[1]) / 2, wz = (wl.z[0] + wl.z[1]) / 2;
  const list = [
    // butterflies: anchor, orbit radius, half-span (m), colour
    { k: KIND.butterfly, at: [st.cx, 1.3, st.cz], R: 1.3, s: 0.046, c: lin(GOLD.light, 1.5) },
    { k: KIND.butterfly, at: [st.cx + 0.2, 1.75, st.cz + 0.1], R: 0.85, s: 0.04, c: lin(GOLD.rose, 1.35) },
    { k: KIND.butterfly, at: [2.8, 2.05, -9.3], R: 0.55, s: 0.038, c: lin(GOLD.mid, 1.6) },
    { k: KIND.butterfly, at: [wlx + 0.4, 1.8, wz - 0.25], R: 0.45, s: 0.044, c: lin(GOLD.light, 1.5) },
    { k: KIND.butterfly, at: [wrx - 0.3, 1.6, wz - 0.3], R: 0.5, s: 0.04, c: lin(GOLD.aqua, 1.2) },
    { k: KIND.butterfly, at: [-1.6, 1.9, 6.6], R: 0.7, s: 0.038, c: lin(GOLD.cream, 1.45) },
    // crowns: hover + turn gently toward the viewer (the stage crown "crowns" the hero sneaker)
    { k: KIND.crown, at: [st.cx, 0.93, st.cz], R: 0, s: 0.068, c: lin(GOLD.light, 1.8) },
    { k: KIND.crown, at: [wlx, 2.22, wz], R: 0, s: 0.066, c: lin(GOLD.light, 1.6) },
    { k: KIND.crown, at: [wrx, 2.22, wz], R: 0, s: 0.066, c: lin(GOLD.light, 1.6) },
  ];
  const n = list.length;
